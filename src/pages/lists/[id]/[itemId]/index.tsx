...
function ItemPage() {
    ...
    if (isPending || allTags.isPending || allMedia.isPending) return (
        <div className="flex justify-center items-center h-screen">
            <Spinner />
        </div >)
    if (itemQuery.error?.message == 'Not Found') return <Error404 />
    if (!isSuccess || !allTags.isSuccess || !allMedia.isSuccess) return <ErrorPage message="Failed To Fetch The Item" />

    const itemTags = allTags.data.filter(tag => item.tags.includes(tag.id))
    setupMediaCache(allMedia.data)
