...
import { Card, Image, Input, Modal, ModalBody, ModalContent } from "@heroui/react"
import { useContext, useMemo, useState } from "react"
import { BiSearch } from "react-icons/bi"
import { ItemFormContext, ItemFormCounterGen, ItemFormMedia } from "../../ItemFormProvider"
...
export default function SelectImageModal({
    ...
    const { list, media, item, itemForm } = useContext(ItemFormContext)
    ...
    const filteredMedia = useMemo(() => {
        ...
        return images.filter(image =>
            ...
            image?.keywords?.some(key => typeof key === 'string' && key?.toLowerCase() === value)
        )
                ...
                {(onClose) => (
                    <ModalBody className="w-full p-3">
                        ...
                        <div className="w-full grid grid-cols-sm-card gap-2">
                            <AddImage onClose={onClose} set={set} />
