...
export default function ItemFormLayoutTabs({
    ...
    function deleteTab(index: number) {
        ...
        if (fields.flat().some(field => field.type === "tags"))
            setLayoutTabs(prev => {
                let newArray = [...prev]
                newArray[index === 0 ? 1 : 0][1].push({ id: Date.now().toString(), type: "tags" })
                return newArray
            })
    ...
    return (
        <>
            <Tabs
                ...
                onSelectionChange={(index) => setActiveTabIndex(parseInt(String(index)))}
            >
