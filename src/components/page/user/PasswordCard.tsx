...
export default function UserPagePasswordCard() {
    ...
    function onSubmit() {
        ...
        mutation.mutate({ oldPassword: oldPass, newPassword: newPass })
    }
            ...
            <Card className="w-full ltr:pl-3 rtl:pr-3 bg-accented/70 flex flex-row items-center gap-x-2 duration-200 hover:bg-accented/90">
                <CardBody className="flex flex-row items-center gap-x-3">
                    ...
                    <span className="text-lg flex flex-row gap-x-2 items-center flex-grow">
                        Password: <span className="text-foreground-500">*********</span>
                    </span>
                    {editMode &&
                        <StatusSubmitButton
            ...
            {editMode &&
                <Card className="w-full grid gap-y-2 my-2 p-2 bg-accented/30 animate-fade-in">
                    ...
                    <Input
                        ...
                        isInvalid={Boolean(error?.cause?.newPassword)}
                        color={error?.cause?.newPassword ? "danger" : undefined}
                        errorMessage={error?.cause?.newPassword}
                    />
                </Card>
